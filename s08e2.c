...
int main()
{
    ...
    //alloco dinamicamente il vettore studenti e temp_due
    struct stud *student;
    student=(struct stud*)malloc(dim*sizeof(struct stud));
    temp_due=(int *)malloc(dim*sizeof(int));
    ...
    choose='y';
    choose_es='y';
    //eseguzione ciclo while finchè vuoi inserire un altro studente
    while (choose=='y'||choose=='Y')
    {
        choose='y';
        choose_es='y';
        //Inserisci il numero di matricola
        printf("Insert the registration number (student n.%d) ",i+1);
        ...
        while (k<NUM_ESAMI&&(choose_es=='y'||choose_es=='Y')){
            ...
            while (isdigit(student[i].course[k].vote)!=0||(student[i].course[k].vote>30||student[i].course[k].vote<0)){
                printf("\n\t%s vote not valid, reinsert: ",student[i].course[k].ins);
                scanf(" %d",&student[i].course[k].vote);
                fflush(stdin);
            }
        ...
        // Se hai inserito dim (incrementato o non) studenti e vuoi aggiungerne un altro/altri raddoppia la dimensione del vettore studenti (allocato dinamicamente)
        if (i==dim&&(choose!='y'||choose!='Y')){
            dim=dim*2;
            student=(struct stud*)realloc(student,dim*sizeof(struct stud));
            //Se student==0 non c'è spazio in memoria
            if (student==0){
                printf("\nNo memory left\n");
                break;
            }
            temp_due=(int*)realloc(temp_due,dim*sizeof(int));
            //Se temp_due==0 non c'è spazio in memoria
            if (temp_due==0){
                printf("\nNo memory left\n");
                break;
            }
